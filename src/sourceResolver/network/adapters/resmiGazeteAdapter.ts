// Resmî Gazete arama adaptörü (Sprint 12, madde 7).
...
const PROVIDER_ID = 'resmi-gazete';

export const resmiGazeteAdapter: ProviderSearchAdapter = {
  providerId: PROVIDER_ID,

  canSearch(document: Document): boolean {
    return document.institution === 'Resmî Gazete';
  },
  ...
  buildSearchRequests(document: Document): readonly NetworkSearchRequest[] {
    ...
    return [{ documentId: document.id, providerId: PROVIDER_ID, url: target, method: 'GET', reason: 'Resmî Gazete/mevzuat.gov.tr sayfa taraması' }];
  },
